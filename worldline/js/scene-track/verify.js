...
async function run() {
  ...
  const server = spawn('python3', [
    ...
    '8889',
    '--directory',
    '/Users/josephbailey/josephbaileyy.github.io',
  ]);
  ...
  async function checkPixels(p, label) {
    ...
    console.log(
      `[${label}] Runner dot is at screen X: ${pixelData.dotX.toFixed(1)}, Y: ${pixelData.dotY.toFixed(1)} (Expected X: ${pixelData.expectedX.toFixed(1)})`,
    );
  ...
  await page.evaluate(async () => {
    ...
    viewport.innerHTML = '';
    const { createScene } = await import('./index.js');
    const reducedScene = createScene(viewport, { reducedMotion: true });
    reducedScene.onProgress(1);
  });
  await page.waitForSelector('.track-scene-container.reduced-motion');
