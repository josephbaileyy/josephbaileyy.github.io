...
interface ChunkMeta {
  ...
  startUnixMs?: number;
  file: string;
  bodies: Record<EphemerisBody, BodyLayout>;
}
interface Manifest {
  version: number;
  source: string;
...
export class EphemerisProvider extends EventTarget {
  ...
  async loadFor(utcMs: number): Promise<void> {
    ...
    try {
      ...
      if (!meta)
        throw new Error(`date ${new Date(utcMs).getUTCFullYear()} outside ephemeris range`);
      ...
      this.pending -= 1;
      if (this.pending === 0 && this.status !== 'fallback')
        this.status = this.loaded.size ? 'ready' : 'idle';
  ...
  private async getManifest(): Promise<Manifest> {
    ...
    try {
      this.manifest = await this.manifestJob;
    } catch (error) {
  ...
  state(
    ...
    const vy =
      (dh00 * data[a + 1] + dh01 * data[b + 1]) / h + dh10 * data[a + 4] + dh11 * data[b + 4];
    const vz =
      (dh00 * data[a + 2] + dh01 * data[b + 2]) / h + dh10 * data[a + 5] + dh11 * data[b + 5];
  ...
  private fallback(
    ...
    if (body === 'moon') {
      ...
      position.add(
        new Vector3(Math.cos(phase), 0.08 * Math.sin(phase * 0.9), -Math.sin(phase)).multiplyScalar(
          0.00257,
        ),
      );
    } else {
