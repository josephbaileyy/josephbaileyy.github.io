...
const notes = JSON.parse(
  await readFile(new URL('../src/content/notes.json', import.meta.url), 'utf8'),
...
const links = portfolio.links
  ...
  .join('\n');
const socialCards = portfolio.socials
  .map((social) => {
    ...
    const copy = social.copyText
      ...
      : '';
    return `<article class="social-card"><span>${escapeHtml(social.display)}</span><h3>${escapeHtml(social.label)}</h3><strong>${escapeHtml(social.handle)}</strong><p>${escapeHtml(social.description)}</p><div>${socialLinks} ${copy}</div></article>`;
  })
  .join('\n');
const noteCards = notes
  .map(
    (note) =>
      `<article class="card"><h3><a href="/notes/${escapeHtml(note.slug)}.html">${escapeHtml(note.title)}</a></h3><p>${escapeHtml(note.summary)}</p><span class="meta">${escapeHtml(note.date)}</span></article>`,
  )
  .join('\n');
const currentCards = [
  {
...
<html lang="en">
  <head>
    ...
    <style>
      :root{--bg:#0b1026;--panel:#131a3a;--panel-2:#171f46;--ink:#eef2ff;--dim:#aeb7d9;--gold:#ffd479;--cyan:#7fd4ff;--violet:#8f7fff}*{box-sizing:border-box}html{scroll-behavior:smooth;scroll-padding-top:72px}body{margin:0;background:radial-gradient(circle at 50% -20%,#1b1740 0,var(--bg) 38%);color:var(--ink);font-family:"Avenir Next","Segoe UI",system-ui,-apple-system,sans-serif;line-height:1.7}a{color:var(--cyan)}.skip{position:absolute;left:16px;top:-80px;background:var(--ink);color:var(--bg);padding:9px 14px;border-radius:8px;z-index:3}.skip:focus{top:12px}.section-nav-shell{position:sticky;top:0;z-index:2;border-bottom:1px solid rgb(127 212 255/.14);background:rgb(11 16 38/.9);backdrop-filter:blur(10px)}.section-nav{display:flex;gap:4px;overflow-x:auto;padding:10px max(18px,calc((100vw - 840px)/2));scrollbar-width:none}.section-nav::-webkit-scrollbar{display:none}.section-nav a{flex:0 0 auto;padding:6px 10px;border-radius:999px;color:var(--dim);font:12px ui-monospace,Menlo,monospace;text-decoration:none}.section-nav a:hover,.section-nav a:focus-visible{background:var(--panel);color:var(--ink);outline:1px solid var(--cyan)}.section-nav a[aria-current="location"]{background:var(--panel);color:var(--gold);outline:1px solid rgb(255 212 121/.45)}main{max-width:840px;margin:0 auto;padding:56px 24px 96px}header{max-width:720px}h1{font-size:clamp(38px,7vw,56px);margin:0 0 4px;line-height:1.08}.tagline{color:var(--dim);font:14px/1.6 ui-monospace,Menlo,monospace;margin:0 0 24px}h2{font:700 14px ui-monospace,Menlo,monospace;letter-spacing:.16em;text-transform:uppercase;color:var(--cyan);margin:52px 0 12px;border-bottom:1px solid rgb(127 212 255/.2);padding-bottom:8px}h3{margin:0 0 6px;font-size:17px;line-height:1.4}.links{display:flex;flex-wrap:wrap;gap:10px;margin:22px 0 6px}.links a{text-decoration:none;border:1px solid rgb(174 183 217/.4);border-radius:999px;padding:8px 16px;font-size:14px;color:var(--ink);background:var(--panel)}.links a:hover,.links a:focus-visible{border-color:var(--cyan);outline:2px solid transparent}.links a.primary{background:var(--gold);color:#1a1430;border-color:var(--gold);font-weight:700}.current-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.current-card,.card,.scale-card,.social-card{background:linear-gradient(145deg,var(--panel-2),var(--panel));border:1px solid rgb(174 183 217/.2);border-radius:14px;padding:20px}.current-card span,.scale-card span,.social-card span{color:var(--gold);font:11px ui-monospace,Menlo,monospace;letter-spacing:.14em;text-transform:uppercase}.current-card p,.card p,.scale-card p,.social-card p{margin:0;color:#d4daf2;font-size:15px}.current-card p{display:-webkit-box;overflow:hidden;-webkit-box-orient:vertical;-webkit-line-clamp:4}.current-card a{display:inline-block;margin-top:12px;font-size:13px}.card{margin:14px 0}.card.featured{border-left:3px solid var(--gold);padding-left:24px}.scale-grid,.social-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:14px}.scale-card{border-color:rgb(143 127 255/.25)}.scale-card small{display:block;color:var(--dim);font:12px/1.5 ui-monospace,Menlo,monospace;margin-top:9px}.scale-card ul{margin:12px 0 0;color:#d4daf2;font-size:13px}.social-card{border-color:rgb(127 212 255/.18)}.social-card strong{display:block;color:var(--cyan);font:12px/1.5 ui-monospace,Menlo,monospace;overflow-wrap:anywhere}.social-card code{display:inline-block;margin-top:8px;border:1px solid rgb(255 212 121/.22);border-radius:999px;background:rgb(255 212 121/.1);color:var(--gold);font:12px/1.4 ui-monospace,Menlo,monospace;padding:5px 9px}.evidence{display:grid;gap:9px;margin:16px 0}.evidence div{display:grid;grid-template-columns:76px minmax(0,1fr);gap:12px}.evidence dt{color:var(--gold);font:11px/1.5 ui-monospace,Menlo,monospace;letter-spacing:.1em;text-transform:uppercase}.evidence dd{margin:0;color:#d4daf2;font-size:14px;line-height:1.55}.tools,.meta{font:12px/1.5 ui-monospace,Menlo,monospace;display:block;margin-top:9px}.tools{color:var(--cyan)}.meta{color:var(--dim)}.universe-invite{margin-top:64px;padding:22px;border-radius:14px;background:linear-gradient(160deg,#1b1740,#0d1130);border:1px solid rgb(127 212 255/.25);text-align:center}ul{padding-left:22px}footer{margin-top:64px;color:var(--dim);font:13px/1.6 ui-monospace,Menlo,monospace}@media(max-width:620px){main{padding:38px 20px 72px}.current-grid,.scale-grid,.social-grid{grid-template-columns:1fr}.card,.current-card,.scale-card,.social-card{padding:17px}.card.featured{padding-left:19px}.evidence div{grid-template-columns:1fr;gap:2px}.links a{min-height:44px;display:inline-flex;align-items:center}.section-nav{padding-inline:10px;padding-right:44px}.section-nav-shell::after{content:"";position:absolute;right:0;top:0;bottom:0;width:42px;background:linear-gradient(90deg,transparent,rgb(11 16 38/.98));pointer-events:none}}@media(prefers-reduced-motion:reduce){html{scroll-behavior:auto}}
    </style>
  </head>
  <body>
    ...
    <div class="section-nav-shell">
      <nav class="section-nav" aria-label="Portfolio sections">
        <a href="#current">Current</a><a href="#scale">Scale</a><a href="#research">Research</a><a href="#experience">Experience</a><a href="#projects">Projects</a><a href="#writing">Writing</a><a href="#socials">Socials</a><a href="#education">Education</a><a href="#beyond">Beyond</a>
      </nav>
    </div>
    <main>
      <header>
      ...
      <section id="writing" aria-labelledby="writing-title"><h2 id="writing-title">Writing</h2>${noteCards}<p><a href="/notes/">All notes →</a></p></section>
      <section id="socials" aria-labelledby="socials-title"><h2 id="socials-title">Personal orbit</h2><div class="social-grid">${socialCards}</div></section>
    ...
    <script
      data-goatcounter="https://josephbaileyy.goatcounter.com/count"
      async
