...
describe('JPL ephemeris interpolation', () => {
    ...
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ...
        json: async () => ({
          ...
          source: 'test',
          chunks: [
            { startYear: 1970, endYear: 1970, startJd, startUnixMs: 0, file: '/chunk.bin', bodies },
          ],
        }),
      })
  ...
  it('matches a stored Horizons Mercury reference vector', async () => {
    const manifestBytes = await readFile(
      new URL('../public/ephemeris/manifest.json', import.meta.url),
    );
    const chunkBytes = await readFile(
      new URL('../public/ephemeris/2025-2029.bin', import.meta.url),
    );
    ...
    expect(position.y).toBeCloseTo(-0.01370545, 7);
    expect(position.z).toBeCloseTo(0.409217014, 7);
  });

  it('keeps the displayed chunk accurate while the next time range buffers', async () => {
    const layouts = Object.fromEntries(
      EPHEMERIS_BODIES.map((body) => [body, { offset: 0, count: 2, stepDays: 1 }]),
    );
    ...
    const delayed = new Promise<{ ok: boolean; arrayBuffer: () => Promise<ArrayBuffer> }>(
      (resolve) => {
