...
describe('portfolio content', () => {
  it('keeps every major section available inside the interactive site', () => {
    expect(Object.keys(PANELS)).toEqual(expect.arrayContaining(['profile', 'research', 'projects', 'am-cvn']));
  ...
  it('includes valid primary profile links', () => {
    ...
    expect(hrefs).toEqual(expect.arrayContaining(['/resume.pdf', 'https://github.com/josephbaileyy']));
    expect(hrefs.every((href) => href.startsWith('/') || href.startsWith('http') || href.startsWith('mailto:'))).toBe(true);
  });
});
